...
export function DetailedHelpCenterModal({
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white p-0 text-slate-900 shadow-2xl dark:border-slate-800 dark:bg-[#0B192C] dark:text-white">
                {/* Header Banner */}
                <div className="relative shrink-0 overflow-hidden bg-gradient-to-r from-[#0b1c5c] via-[#1e3a8a] to-[#2563eb] px-6 py-6 text-white sm:px-8">
                    ...
                    <div className="relative flex flex-col gap-4">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                <div className="grid h-11 w-11 place-items-center rounded-2xl bg-white/10 text-white backdrop-blur-md shadow-inner">
                                ...
                                <div>
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Badge className="bg-blue-400/20 text-blue-200 border-blue-300/30 text-[10px] font-bold uppercase tracking-wider">
                                            {role === 'admin' ? 'Administrator' : role === 'program_head' ? 'Program Head' : 'Student Portal'}
                                        </Badge>
                                    </div>
                                    <DialogDescription className="mt-0.5 text-xs text-blue-100/80">
                        ...
                        <div className="relative mt-1">
                            <Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-blue-200/70" />
                            <Input
                                placeholder="Search guides, step-by-step instructions, thermal printing, QR scans..."
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                                className="h-11 w-full rounded-xl border-white/20 bg-white/10 pl-10 pr-4 text-xs font-medium text-white placeholder:text-blue-200/60 backdrop-blur-md focus:border-white focus:bg-white/15 focus:ring-2 focus:ring-white/20"
                            />
                            {searchQuery && (
                                <button
                                    type="button"
                ...
                <div className="shrink-0 border-b border-slate-200/80 bg-slate-50/80 px-6 py-2.5 dark:border-slate-800 dark:bg-slate-900/60 overflow-x-auto">
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-bold transition-all ${
                                        isActive
                                            ...
                                            : 'text-slate-600 hover:bg-slate-200/60 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200'
                                    }`}
                                >
                ...
                <div className="flex-1 overflow-y-auto min-h-0 space-y-6 p-5 sm:p-6 bg-slate-50/40 dark:bg-slate-950/30">
                            ...
                            <div className="space-y-3">
                                {filteredGuides.map((guide) => {
                                    ...
                                    return (
                                        <div
                                            key={guide.id}
                                            className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm transition-all duration-200 hover:border-blue-300 dark:border-slate-800 dark:bg-[#0B192C]/70 dark:hover:border-blue-500/50"
                                        >
                                            <button
                                                type="button"
                                                onClick={() =>
                                                ...
                                                className="flex w-full items-start justify-between gap-3 p-4 text-left transition-colors hover:bg-slate-50/50 dark:hover:bg-slate-800/30"
                                            >
                                                <div className="flex-1">
                                                    <div className="flex items-center gap-2">
                                                        {guide.badge && (
                                                            <Badge className="bg-blue-50 text-blue-700 border-blue-200 text-[10px] font-bold dark:bg-blue-950/40 dark:text-blue-300 dark:border-blue-800">
                                                                {guide.badge}
                                                            </Badge>
                                                        )}
                                                ...
                                                <div className="ml-2 mt-1 rounded-lg bg-slate-100 p-1 text-slate-500 dark:bg-slate-800 dark:text-slate-400">
                                                    {isExpanded ? (
                                            ...
                                            {isExpanded && (
                                                <div className="border-t border-slate-100 bg-slate-50/50 p-4 dark:border-slate-800/80 dark:bg-slate-900/40 space-y-4">
                                                    {guide.steps && (
                                                        <div className="space-y-2">
                                                            ...
                                                            <div className="space-y-2">
                                                                {guide.steps.map((step, idx) => (
                                                                    <div
                                                                        key={idx}
                                                                        className="flex items-start gap-2.5 rounded-xl bg-white p-2.5 text-xs text-slate-700 shadow-xs dark:bg-slate-800/60 dark:text-slate-200 border border-slate-200/60 dark:border-slate-700/60"
                                                                    >
                                                    ...
                                                    {guide.tips && (
                                                        <div className="flex items-start gap-2.5 rounded-xl border border-amber-200 bg-amber-50/70 p-3 text-amber-800 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-300">
                                    ...
                                    return (
                                        ...
                                        >
                                            <button
                                                ...
                                                className="flex w-full items-center justify-between gap-3 p-3.5 text-left text-xs font-bold text-slate-800 hover:bg-slate-50/50 dark:text-slate-200 dark:hover:bg-slate-800/30"
                                            >
                                                <div className="flex items-center gap-2">
                                            ...
                                            {isFaqExpanded && (
                                                <div className="border-t border-slate-100 bg-slate-50/50 p-3.5 text-xs text-slate-600 leading-relaxed dark:border-slate-800 dark:bg-slate-900/40 dark:text-slate-300">
                    ...
                    {filteredGuides.length === 0 && filteredFaqs.length === 0 && (
                        <div className="rounded-2xl border border-slate-200/80 bg-white p-8 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
                ...
                {/* Footer Actions */}
                <div className="flex items-center justify-between border-t border-slate-200/80 bg-slate-50 px-6 py-3.5 dark:border-slate-800 dark:bg-slate-900/80">
                    ...
                    <Button
                        ...
                        className="h-9 rounded-xl border-slate-300 text-xs font-semibold dark:border-slate-700"
                    >
                        Close Help Center
                    </Button>
                </div>
