...
export default function GoogleMapSelector({ onLocationSelect, initialLocation }: GoogleMapSelectorProps) {
  ...
  const [manualLat, setManualLat] = useState(initialLocation?.lat?.toFixed(6) ?? '');
  const [manualLng, setManualLng] = useState(initialLocation?.lng?.toFixed(6) ?? '');
  ...
  const validateApiKey = () => {
    if (!GOOGLE_MAPS_API_KEY) {
      console.warn('No API key provided. See GOOGLE_MAPS_SETUP.md for instructions.');
      return false;
    }
  ...
  useEffect(() => {
    ...
    if (!validateApiKey()) {
      console.warn('Google Maps API key is missing or invalid - using fallback mode');
      console.log(`
        === GOOGLE MAPS SETUP ===
        ...
        ========================
      `);
      setError('Google Maps API key is missing or invalid. Using manual entry mode. See GOOGLE_MAPS_SETUP.md for instructions.');
      return;
    }
    ...
    const timeout = setTimeout(() => {
      if (!window.google) {
        ...
        setError('Google Maps failed to load. Check your network connection and API key. Using manual entry mode.');
      }
    }, 10000); // 10 seconds
    ...
    window.gm_authFailure = () => {
      ...
      console.log(`
        ...
        ==============================
      `);
      setError('Google Maps authentication failed. Check your API key and restrictions. Using manual entry mode.');
    };

    script.onerror = (event: string | Event) => {
      clearTimeout(timeout);
      console.error('Google Maps script error:', event);
      setError('Failed to load Google Maps. Check API key and ensure Maps JavaScript API is enabled. Using manual entry mode.');
  ...
  useEffect(() => {
    ...
    const map = new window.google.maps.Map(mapRef.current, {
      center: initialLocation ? { lat: initialLocation.lat, lng: initialLocation.lng } : { lat: 14.5995, lng: 120.9842 },
      zoom: 16,
      mapTypeId: 'roadmap',
    });

    const marker = new window.google.maps.Marker({
      position: initialLocation ? { lat: initialLocation.lat, lng: initialLocation.lng } : { lat: 14.5995, lng: 120.9842 },
      map,
      draggable: true,
    ...
    autocomplete.addListener('place_changed', () => {
      ...
      const name = place.formatted_address || `${lat.toFixed(6)}, ${lng.toFixed(6)}`;
  ...
  }, [isScriptLoaded, initialLocation, onLocationSelect, selectedLocationName]);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-lg overflow-hidden border border-slate-300" style={{ height: '300px' }}>
        ...
        <div ref={mapRef} className="w-full h-full" style={{ display: isScriptLoaded && !error ? 'block' : 'none' }} />
      </div>

      {!selectedLocationName && !error && isScriptLoaded && (
        <p className="text-xs text-amber-600">Please select a location on the map or via search.</p>
      )}
          ...
          <div className="mt-2 p-3 bg-slate-50 rounded-md space-y-2">
            ...
            {error && (
              <p className="text-xs text-slate-500 mt-2">
                Map is unavailable due to API issues. Use manual entry to continue testing geotagging.
              </p>
            )}
          </div>
        )}
