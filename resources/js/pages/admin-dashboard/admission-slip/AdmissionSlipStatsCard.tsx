    ...
    return (
        <div className="mb-6 grid gap-4 lg:grid-cols-3">
            {kpis.map((kpi, index) => (
                ...
                >
                    <div
                        className={`pointer-events-none absolute -top-4 -right-4 h-24 w-24 rounded-full ${kpi.glow}`}
                    />
                    <div className="flex items-start justify-between gap-3">
                        <div>
                            ...
                            <p
                                className={`mt-1 text-xs font-semibold ${kpi.subtextColor}`}
                            >
                                {kpi.subtext}
                            </p>
                        </div>
                        <div
                            className={`grid h-12 w-12 shrink-0 place-items-center rounded-2xl ${kpi.iconBg} transition-transform duration-300 group-hover:scale-110`}
                        >
                    ...
                    <div className="mt-4 h-1 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                        <div
                            className={`h-full w-full bg-gradient-to-r ${kpi.barGradient} rounded-full`}
                        />
                    </div>
                </div>
            ))}
