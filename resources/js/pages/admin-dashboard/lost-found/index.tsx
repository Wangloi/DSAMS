...
import { Archive, CheckCircle2, Edit, ScrollText, Search, Trash2, X, RefreshCw, Pause, Play } from 'lucide-react';
...
import { adminDashboard, adminLostFound, adminLostFoundStore, adminLostFoundUpdate, adminLostFoundDestroy, adminLostFoundArchive } from '@/routes';
...
export default function AdminLostFoundPage() {
    ...
    const [filter, setFilter] = useState<'all' | 'unclaimed' | 'claimed' | 'pending' | 'storage'>('all');
    ...
    const kpis = useMemo(() => [
        { title: 'Total Items', value: stats?.totalItems ?? 0, change: '', accent: 'bg-blue-600' },
        { title: 'Unclaimed Items', value: stats?.unclaimed ?? 0, change: '', accent: 'bg-amber-500' },
        { title: 'Pending Items', value: stats?.pending ?? 0, change: '', accent: 'bg-sky-500' },
        { title: 'Claimed Items', value: stats?.claimed ?? 0, change: '', accent: 'bg-emerald-600' },
    ], [stats]);
    ...
    const filteredItems = useMemo(() => {
        ...
        if (filter !== 'all') {
            ...
            filtered = filtered.filter((item: FoundItemRow) => item.status === statusMap[filter as keyof typeof statusMap]);
        }

        // Apply search filter
        if (query.trim()) {
            const searchLower = query.toLowerCase().trim();
            filtered = filtered.filter((item: FoundItemRow) =>
    ...
    const handleArchive = (item: FoundItemRow) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.put(adminLostFoundArchive(id), {}, {
                    preserveScroll: true,
                    onSuccess: () => {
                        Swal.fire({
                            icon: 'success',
            ...
            <div className="min-h-[calc(100vh-4rem)] bg-slate-100 dark:bg-slate-900">
                <div className="flex w-full flex-col gap-6 px-6 py-6">
                        ...
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="leading-tight">
                                    ...
                                    <div className="text-sm text-white/80">Track found items and claim status</div>
                                </div>
                            </div>
                            <div className="flex flex-wrap items-center gap-3">
                                <div className="hidden sm:flex sm:items-center sm:gap-3 mr-2 text-right">
                                    <div>
                                        <div className="text-[10px] font-bold uppercase tracking-wider text-white/70 flex items-center justify-end gap-1">
                                            {isRefreshing && <RefreshCw className="h-3 w-3 animate-spin text-white" />}
                                    ...
                                    <Button
                                        variant="ghost"
                                        size="icon"
                                        className="h-10 w-10 shrink-0 rounded-xl bg-white/10 text-white hover:bg-white/20 transition-all"
                                ...
                                <AddFoundItemDialog editingItem={editingItem} setEditingItem={setEditingItem} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                        {kpis.map((kpi) => (
                            <Card key={kpi.title} className="overflow-hidden border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-[#0B192C]/50">
                                <CardContent className="relative py-4">
                                    <div className="flex items-start justify-between gap-3">
                                        <div>
                                            <div className="text-xs font-medium text-slate-600 dark:text-slate-400">{kpi.title}</div>
                                            <div className="mt-1 flex items-end gap-2">
                                                <div className="text-2xl font-semibold leading-none text-slate-900 dark:text-white">{kpi.value}</div>
                                                {kpi.change ? (
                                                    <div className="text-sm font-semibold text-emerald-700 dark:text-emerald-400">{kpi.change}</div>
                                                ) : null}
                                            </div>
                                        </div>
                    ...
                    <Card className="border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-[#0B192C]/50">
                        <CardHeader className="pb-3">
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    ...
                                    <div className="mt-1 text-xs text-slate-500">Showing {filteredItems.length} results</div>
                                </div>
                                <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                                    ...
                                    <div className="relative w-full sm:w-80">
                                        ...
                                        <Input
                                            ...
                                            className="h-10 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 pl-9 pr-10 dark:text-white"
                        ...
                        <CardContent className="pt-0">
                            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                                {pagedItems.length === 0 ? (
                                    <div className="col-span-full rounded-2xl border-2 border-dashed border-slate-200 py-12 text-center">
                                        ...
                                        <div className="mt-3 text-sm font-medium text-slate-900">No items found</div>
                                        <div className="mt-1 text-[11px] text-slate-500 text-balance">Try adjusting your filters or search query</div>
                                    </div>
                                ) : (
                                    pagedItems.map((item: FoundItemRow) => (
                                        <Card key={item.id} className="group h-fit overflow-hidden border-slate-200 bg-white shadow-none transition-all duration-300 hover:shadow-lg hover:translate-y-[-2px] dark:border-slate-800 dark:bg-[#0B192C]/50">
                                            <div className="relative aspect-video overflow-hidden bg-slate-100 dark:bg-slate-800">
                                                ...
                                                <div className="absolute left-2 top-2">
                                                    <span className={`inline-flex rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-white shadow-md backdrop-blur-sm ${statusBadge(item.status)}`}>
                                                        {item.status}
                                                    </span>
                                                </div>
                                            </div>
                                            <CardContent className="p-3.5">
                                                <h3 className="line-clamp-1 h-5 text-sm font-bold text-slate-900 group-hover:text-[#23509A] transition-colors">{item.title}</h3>
                                                
                                                <div className="mt-3 flex flex-col gap-1.5 border-t border-slate-50 pt-3 text-[11px]">
                                                    <div className="flex items-center gap-2">
                                                        <CheckCircle2 className="h-3 w-3 text-slate-400" />
                                                        <span className="truncate font-medium text-slate-600">Found: {item.foundAt}</span>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                        <Search className="h-3 w-3 text-slate-400" />
                                                        <span className="truncate font-medium text-slate-600" title={item.location}>{item.location}</span>
                                                    </div>
                                                </div>

                                                <div className="mt-4 flex items-center justify-between gap-1.5 pt-3 border-t border-slate-50">
                            ...
                            <div className="mt-3 flex flex-col gap-2 text-xs text-slate-600 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    Showing {filteredItems.length === 0 ? 0 : (Math.min(Math.max(pageIndex, 1), totalPages) - 1) * pageSize + 1} to{' '}
                                    {Math.min(Math.min(Math.max(pageIndex, 1), totalPages) * pageSize, filteredItems.length)} of {filteredItems.length} entries
                                </div>
