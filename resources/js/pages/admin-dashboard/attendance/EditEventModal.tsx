...
export default function EditEventModal({
    ...
    const [scannerStudentIdInput, setScannerStudentIdInput] =
        useState<string>('');
    ...
    useEffect(() => {
        if (editingEvent) {
            ...
            setFormData({
                ...
                eventTime: time,
                registrationEndTime: String(
                    editingEvent.registrationEndTime ?? '',
                ),
                scannerStudentIds: Array.isArray(editingEvent.scannerStudentIds)
                    ? editingEvent.scannerStudentIds
                    : [],
                expectedAttendees:
                    editingEvent.expectedAttendees !== undefined &&
                    editingEvent.expectedAttendees !== null
                        ? String(editingEvent.expectedAttendees)
                        : String(editingEvent.totalAttendees ?? ''),
                description: '',
                courses: Array.isArray(editingEvent.courses)
                    ? editingEvent.courses.map((c) =>
                          typeof c === 'string' ? c : c.id,
                      )
                    : [],
                yearLevels: Array.isArray(editingEvent.year_levels)
                    ? editingEvent.year_levels.map((yl) =>
                          typeof yl === 'string' ? yl : yl.id,
                      )
                    : [],
                geofenceEnabled: Boolean(
                    (editingEvent as any)?.geofenceEnabled ?? false,
                ),
                geofenceLatitude: String(
                    (editingEvent as any)?.geofenceLatitude ?? '',
                ),
                geofenceLongitude: String(
                    (editingEvent as any)?.geofenceLongitude ?? '',
                ),
                geofenceRadiusM: String(
                    (editingEvent as any)?.geofenceRadiusM ?? 50,
                ),
                ...
                <div className="bg-gradient-to-r from-[#0b2d66] to-[#1e40af] px-6 py-5 text-white">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-white/80">
                            Update the event details to modify the attendance
                            event.
                        </DialogDescription>
                    </DialogHeader>
                </div>
                    ...
                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
                        <div className="grid gap-4 sm:grid-cols-2 lg:col-span-8">
                            ...
                            <div className="grid gap-2 sm:col-span-2">
                                ...
                                {formData.scannerStudentIds.length > 0 ? (
                                    <div className="mt-2 flex flex-wrap gap-2">
                                        {formData.scannerStudentIds.map(
                                            (id) => (
                                                <button
                                                    key={id}
                                                    type="button"
                                    ...
                                    <div className="text-xs text-slate-400 italic">
                                        No allowed scanner students added.
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="grid gap-2">
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    <span className="text-xs text-slate-500">
                                        From event date (before today =
                                        completed, today = ongoing, after =
                                        upcoming).
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="grid gap-2 sm:col-span-1">
                                ...
                                <div className="space-y-2">
                                    <div className="text-xs text-slate-500">
                                        Select courses that should attend this
                                        event
                                    </div>
                                    ...
                                    <div className="grid max-h-32 grid-cols-1 gap-2 overflow-y-auto rounded-md border p-2">
                                        {courses.map((course) => (
                                            <label
                                                key={course.id}
                                                className="flex cursor-pointer items-center space-x-2 rounded p-1 hover:bg-slate-50"
                                            >
                                                <input
                                                    type="checkbox"
                                    ...
                                    {formData.courses.length === 0 && (
                                        <div className="text-xs text-slate-400 italic">
                                            No courses selected - this event is
                                            open to all courses
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="grid gap-2 sm:col-span-1">
                                ...
                                <div className="space-y-2">
                                    <div className="text-xs text-slate-500">
                                        Select year levels that should attend
                                        this event
                                    </div>
                                    <div className="mb-2 flex items-center space-x-2">
                                        <input
                                    ...
                                    <div className="grid max-h-32 grid-cols-1 gap-2 overflow-y-auto rounded-md border p-2">
                                        {yearLevels.map((yearLevel) => (
                                            ...
                                            >
                                                <input
                                                    type="checkbox"
                                                    checked={formData.yearLevels.includes(
                                                        yearLevel.id,
                                                    )}
                                                    onChange={(e) => {
                                                        if (e.target.checked) {
                                    ...
                                    {formData.yearLevels.length === 0 && (
                                        <div className="text-xs text-slate-400 italic">
                                            No year levels selected - this event
                                            is open to all year levels
                                        </div>
                                    )}
                                </div>
                            </div>
