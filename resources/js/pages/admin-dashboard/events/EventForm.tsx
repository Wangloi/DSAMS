...
import { deriveEventLifecycleStatus, lifecycleStatusBadgeClass } from './deriveEventLifecycleStatus';
        ...
        <div className="space-y-4">
          <div>
            ...
            {showMapSelector && (
              <div className="mt-4 border rounded p-2">
                <SchoolMapSelector
                  ...
                  initialLocation={
                    data.geofence_latitude && data.geofence_longitude
                      ? { latitude: parseFloat(data.geofence_latitude), longitude: parseFloat(data.geofence_longitude), name: selectedLocationName }
                      : undefined
                  }
                />
              </div>
            ...
            {selectedLocationName && (
              <div className="mt-2 text-sm text-gray-600">
                Selected Location: <span className="font-medium">{selectedLocationName}</span>
              </div>
            )}
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              ...
              onCheckedChange={checked => setData('geofence_enabled', !!checked)}
            />
            <Label htmlFor="geofence_enabled" className="text-sm font-normal">Enable Geofence</Label>
          </div>

          {data.geofence_enabled && (
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <div>
                <Label htmlFor="geofence_latitude">Geofence latitude *</Label>
                <Input
                  id="geofence_latitude"
                  value={data.geofence_latitude}
                  onChange={e => setData('geofence_latitude', e.target.value)}
                  placeholder="e.g. 14.5995"
                  className={errors.geofence_latitude ? 'border-red-500' : ''}
                />
                {errors.geofence_latitude && <p className="text-sm text-red-500 mt-1">{errors.geofence_latitude}</p>}
              </div>
              <div>
                <Label htmlFor="geofence_longitude">Geofence longitude *</Label>
                <Input
                  id="geofence_longitude"
                  value={data.geofence_longitude}
                  onChange={e => setData('geofence_longitude', e.target.value)}
                  placeholder="e.g. 120.9842"
                  className={errors.geofence_longitude ? 'border-red-500' : ''}
                />
                {errors.geofence_longitude && <p className="text-sm text-red-500 mt-1">{errors.geofence_longitude}</p>}
              </div>
              <div>
                <Label htmlFor="geofence_radius_m">Radius (meters) *</Label>
                <Input
                  ...
                  onChange={e => setData('geofence_radius_m', e.target.value)}
                  className={errors.geofence_radius_m ? 'border-red-500' : ''}
                />
                {errors.geofence_radius_m && <p className="text-sm text-red-500 mt-1">{errors.geofence_radius_m}</p>}
              </div>
            </div>
          )}
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              ...
              onCheckedChange={checked => setData('scanner_portal_active', !!checked)}
            />
            <Label htmlFor="scanner_portal_active" className="text-sm font-normal">Activate Scanner Portal</Label>
          </div>
        </div>
      )}
              ...
              {mergedCourseChoices.map((opt, idx) => (
                <div key={opt.id} className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={checked => handleCourseChange(opt.id, !!checked)}
                  />
                  <Label htmlFor={`course_${idx}`} className="text-sm font-normal">
            ...
            <div className="grid grid-cols-2 gap-3 mt-2">
              {mergedYearChoices.map((opt, idx) => (
                <div key={opt.id} className="flex items-center space-x-2">
                  <Checkbox
                    id={`year_${idx}`}
                    checked={data.year_levels.includes(opt.id)}
                    onCheckedChange={checked => handleYearLevelChange(opt.id, !!checked)}
                  />
                  <Label htmlFor={`year_${idx}`} className="text-sm font-normal">{opt.name}</Label>
                </div>
              ))}
            </div>
          </div>
