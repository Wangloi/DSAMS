...
import { CheckCircle2, ChevronLeft, Star, ThumbsDown, ThumbsUp, UserRoundCog } from 'lucide-react';
...
export default function AdminEvaluationMetricsPage() {
    ...
    const sentimentCounts = useMemo(() => evaluationStats?.sentiments ?? { positive: 0, neutral: 0, negative: 0 }, [evaluationStats]);
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.post(
                    ...
                    { program },
                    { preserveScroll: true, onSuccess: () => router.reload({ only: ['programStats'] }) },
                );
            }
        });
    };
    ...
    return (
        <AdminLayout breadcrumbs={breadcrumbs}>
            <Head title={`Evaluation Metrics: ${primaryEvaluation?.name || 'Metrics'}`} />
                        ...
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="leading-tight">
                                    <div className="text-sm font-medium text-white/80">Evaluation Metrics</div>
                                    <div className="text-xl font-bold">{primaryEvaluation?.name || 'Evaluation'}</div>
                                    {event && (
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                {primaryEvaluation && !primaryEvaluation.is_active ? (
                                    <Button
                                        type="button"
                                        ...
                                        onClick={() => {
                                            Swal.fire({
                                                ...
                                                if (r.isConfirmed) {
                                                    router.post(adminEvaluationPublish(primaryEvaluation.id), {}, { preserveScroll: true });
                                                }
                                            });
                                        }}
                                    >
                                    ...
                                    <Button
                                        ...
                                        onClick={() => router.post(adminEvaluationUnpublish(primaryEvaluation.id), {}, { preserveScroll: true })}
                                    >
                        ...
                        <div className="lg:col-span-1">
                            <Card className="border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-800">
                                <CardHeader className="pb-3">
                                    ...
                                    <div className="text-sm text-slate-600 dark:text-slate-400">
                                        {completionThreshold}% threshold required
                                    </div>
                                </CardHeader>
                                            ...
                                            >
                                                ...
                                                <div className="mt-3">
                                                    {row.approved ? (
                                                        ...
                                                        <Button
                                                            size="sm"
                                                            onClick={() => handleApproveProgram(row.program)}
                                                        >
                                                            Approve next activity
                                                        </Button>
                                                    ) : (
                                                        <span className="text-xs text-slate-500">
                                                            Below {completionThreshold}% threshold
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
