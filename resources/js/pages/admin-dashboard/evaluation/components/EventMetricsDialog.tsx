...
import { EvaluationForm, EvaluationStats, ProgramStatRow, EventOption } from './types';
...
export default function EventMetricsDialog({
    ...
    const sentimentCounts = evaluationStats?.sentiments ?? { positive: 0, neutral: 0, negative: 0 };
    ...
    return (
        <Dialog open={open} onOpenChange={(val) => !val && onClose()}>
            <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto bg-slate-50 dark:bg-slate-950 border-none p-0">
                ...
                <div className="grid grid-cols-1 gap-6 p-6 lg:grid-cols-2">
                    <Card className="border border-slate-200/70 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 lg:col-span-2">
                        ...
                        <CardContent className="space-y-3">
                            {programStats.length === 0 ? (
                                <p className="text-sm text-slate-500">No program stats available.</p>
                            ) : (
                    ...
                    <Card className="border border-slate-200/70 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 lg:col-span-2">
                        <CardHeader className="pb-3">
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    <div className="text-base font-semibold text-slate-900 dark:text-white">Comments Overview</div>
                                    ...
                                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
                                        ...
                                        <div className="lg:col-span-8 space-y-4">
                                            ...
                                            <div className="mt-6 space-y-3">
                                                <div className="text-sm font-semibold text-slate-900 dark:text-white">Latest Comments</div>
                                                {filteredComments.length === 0 ? (
                                                    <div className="text-sm text-slate-500 dark:text-slate-400">No comments found.</div>
                                                ) : (
                                                    <div className="max-h-64 overflow-y-auto rounded-lg border border-slate-200 dark:border-slate-800">
                                                        {filteredComments.map((comment, index) => (
                                                            <div key={index} className="border-b border-slate-200 p-4 last:border-b-0 dark:border-slate-800">
                                                                <div className="flex items-center justify-between mb-2">
                                                                    <div className="font-semibold text-slate-900 dark:text-white">{comment.student}</div>
                                                                    <div className="flex items-center gap-1">
                                                                        {comment.rating && Array.from({ length: 5 }).map((_, i) => (
                                                                            <Star
                                                                                key={i}
                                                                                className={`h-3 w-3 ${
                                                                                    i < comment.rating!
                                                                        ...
                                                                        <Badge className={`text-xs ${
                                                                            comment.sentiment === 'positive' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' :
                                                                            comment.sentiment === 'negative' ? 'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-300' :
                                                                ...
                                                                <div className="text-sm text-slate-700 dark:text-slate-300">{comment.comment}</div>
