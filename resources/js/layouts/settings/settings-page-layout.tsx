...
export default function SettingsPageLayout({ children, title }: Props) {
    ...
    const backLabel = auth?.roleLabel ? `Back to ${auth.roleLabel} dashboard` : 'Back to dashboard';

    const usesRoleHeader = guard === 'student' || guard === 'program_head' || guard === 'admin';
    ...
    return (
        <>
            ...
            <div className={usesRoleHeader ? 'min-h-screen bg-slate-100 pt-16 dark:bg-slate-950' : 'min-h-screen bg-slate-100 dark:bg-slate-950'}>
